  ...
  try {
    if (sLow.startsWith('create table')) {
      ...
      if (t && tableExists(db, t)) {
        console.info(`skip create table ${t} (exists)`)
        return
      }
    }
    if (sLow.startsWith('alter table') && sLow.includes('add column')) {
      ...
      if (m) {
        ...
        if (columnExists(db, table, col)) {
          console.info(`skip add column ${col} on ${table} (exists)`)
          return
        }
      }
    }
    if (sLow.startsWith('create index') || sLow.startsWith('create unique index')) {
      ...
      if (idx && indexExists(db, idx)) {
        console.info(`skip create index ${idx} (exists)`)
        return
      }
    }
    console.info('apply:', s.split('\n')[0].slice(0, 120))
...
function main() {
  ...
  if (argv.length < 2) {
    console.error('usage: ensure_migrations.cjs <sqlite.db> <migrations_dir>')
    process.exit(2)
  }
  const db = argv[0]
  ...
  for (const f of files) {
    ...
    if (applied.includes(hash)) {
      console.info(`already applied: ${f}`)
      continue
    }
    console.info(`applying: ${f}`)
    applyMigrationFile(db, file)
    markApplied(db, hash)
    console.info(`marked applied: ${f}`)
  }
  console.info('done')
}
