...
export default defineEventHandler(async (event) => {
  ...
  const candidateLogs = await db.select().from(attendanceLog)
  ...
  if (!dayRow) {
    await db.insert(attendanceDay).values({ id: randomUUID(), userId, date: targetDate, selectedShiftCode: bodyShiftCode ?? openClockIn.shiftCode, shiftType: bodyShiftType ?? openClockIn.shiftType ?? undefined, createdAt: now, updatedAt: now })
  }
  ...
  trackServerEvent('attendance.clock-out', { userId, date: targetDate, shiftCode: shiftCodeToPersist, shiftType: shiftTypeToPersist, lat: coords?.latitude, lng: coords?.longitude, earlyReason: earlyReason || null, userAgent: event.node.req.headers['user-agent'], timeZone: bodyTimeZone })
  ...
  return { date: targetDate, requestedDate: currentDateText, logs }
})
